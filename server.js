...
function computeLeaderboard(sessionId, scope = 'total', dayLabel = null, setId = null) {
  // Filter on a.* must live in the JOIN ON, not WHERE, so LEFT JOIN still returns 0-score players.
  let joinFilter = 'a.player_id = p.player_id';
  const params = [];
  if (scope === 'day' && dayLabel) {
    joinFilter += ' AND a.day_label = ?';
    params.push(dayLabel);
  } else if (scope === 'set' && setId) {
    joinFilter += ' AND a.set_id = ?';
    params.push(setId);
  }
  const sql = `
    ...
    FROM players p
    LEFT JOIN answers a ON ${joinFilter}
    WHERE p.session_id = ?
...
io.on('connection', socket => {
  ...
  // Reveal the answer — then auto-emit leaderboard a few seconds later
  socket.on('host_reveal', () => {
    ...
    io.to(roomFor(sessionId, 'host')).emit('answer_revealed', { question_id: q.question_id, correct: q.correct_answer, distribution: dist, total: totalAns, explanation: q.explanation });
    io.to(roomFor(sessionId, 'player')).emit('answer_revealed', { question_id: q.question_id, correct: q.correct_answer, explanation: q.explanation });

    // === Auto-flow: schedule leaderboard reveal ===
    // Determine if this is the last question in its set / activity
    const setQuestions = db.prepare('SELECT question_id FROM questions WHERE set_id=? ORDER BY question_no, question_id').all(q.set_id);
    const isLastInSet = setQuestions.length && setQuestions[setQuestions.length - 1].question_id === q.question_id;

    // Walk full question list to detect end of activity
    const allSets = db.prepare('SELECT set_id FROM question_sets WHERE activity_id=? ORDER BY day_label, sort_order, set_id').all(sess.activity_id);
    let allQuestions = [];
    for (const s of allSets) {
      const list = db.prepare('SELECT question_id FROM questions WHERE set_id=? ORDER BY question_no, question_id').all(s.set_id);
      for (const x of list) allQuestions.push(x.question_id);
    }
    const isLastInActivity = allQuestions.length && allQuestions[allQuestions.length - 1] === q.question_id;

    setTimeout(() => {
      // If the session moved on already (e.g. host clicked Next early), do nothing
      const fresh = db.prepare('SELECT question_phase, current_question_id FROM sessions WHERE session_id=?').get(sessionId);
      if (!fresh || fresh.current_question_id !== q.question_id || fresh.question_phase !== 'reveal') return;

      let scope = 'total';
      let dayLabel = null;
      let podium = false;
      let setIdFilter = null;
      let title = '即時排名';

      if (isLastInActivity) {
        scope = 'total'; podium = true; title = '🏆 總冠軍排行榜';
      } else if (isLastInSet) {
        scope = 'set';  setIdFilter = q.set_id;  podium = true;  title = `${q.set_name} 題組排名`;
      }

      const rows = computeLeaderboard(sessionId, scope, dayLabel, setIdFilter);
      db.prepare('UPDATE sessions SET question_phase=\'leaderboard\' WHERE session_id=?').run(sessionId);
      const payload = { scope, day_label: dayLabel, set_id: setIdFilter, set_name: q.set_name, podium, title, rows };
      io.to(roomFor(sessionId, 'host')).emit('leaderboard', payload);
      io.to(roomFor(sessionId, 'player')).emit('leaderboard', payload);
    }, 4500); // 4.5 sec: time for players to see correct answer + distribution
  });
...
// Bind to 0.0.0.0 explicitly — Render/Fly/Railway require this, not IPv6-only or localhost
server.listen(PORT, '0.0.0.0', () => {
  console.log(`\n  Quiz platform listening on 0.0.0.0:${PORT}`);
  console.log(`  PUBLIC_URL = ${process.env.PUBLIC_URL || '(not set — using request host)'}`);
  console.log('  Routes:  /admin.html  /host.html?s=CODE  /join.html?s=CODE\n');
});

// Surface uncaught errors in Render logs instead of silent crash
